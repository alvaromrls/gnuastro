...
/* See if the tile is on the edge of the hosted region or not. It doesn't
   ...
   channel). */
static int convolve_tile_is_on_edge(size_t *h, size_t *start_end_coord,
                                    size_t *k, size_t ndim) {
...
static int convolve_spatial_overlap(struct per_thread_spatial_prm *pprm,
                                    int tocorrect) {
  ...
  if (tocorrect) {
    ...
    ppf = (pp = pprm->overlap_start) + ndim;
    do
      *pp += *hs++;
    while (++pp < ppf);
  }
  ...
  increment =
      gal_dimension_coord_to_index(ndim, block->dsize, pprm->overlap_start);
  pprm->i_overlap->array =
      gal_pointer_increment(block->array, increment, block->type);

  /* Set the starting point of the kernel overlap tile. */
  increment = (full_overlap ? 0
                            : gal_dimension_coord_to_index(ndim, kernel->dsize,
                                                           pprm->kernel_start));
  pprm->k_overlap->array =
      gal_pointer_increment(kernel->array, increment, kernel->type);
  return full_overlap;
}

/* Convolve over one tile that is not touching the edge. */
static void convolve_spatial_tile(struct per_thread_spatial_prm *pprm) {
  ...
  if (cprm->tocorrect && pprm->on_edge == 0)
    return;
  ...
  while (i_st_en[0] + i_inc <= i_st_en[1]) {
    ...
    /* Increase the increment from the start of the tile for the next
       contiguous patch. */
    i_inc += gal_tile_block_increment(block, tile->dsize, i_ninc++, pprm->pix);
  }
  /*
  if(pprm->id==2053)
    printf("... done.\n");
...
static void *convolve_spatial_on_thread(void *inparam) {
  ...
  size_t *dsize =
      gal_pointer_allocate(GAL_TYPE_SIZE_T, ndim, 0, __func__, "dsize");
  ...
  pprm->cprm = cprm;
  pprm->pix =
      gal_pointer_allocate(GAL_TYPE_SIZE_T, 2 * ndim, 0, __func__, "pprm->pix");
  pprm->host_start = gal_pointer_allocate(GAL_TYPE_SIZE_T, ndim, 0, __func__,
                                          "pprm->host_start");
  pprm->kernel_start = gal_pointer_allocate(GAL_TYPE_SIZE_T, ndim, 0, __func__,
                                            "pprm->kernel_start");
  pprm->overlap_start = gal_pointer_allocate(GAL_TYPE_SIZE_T, ndim, 0, __func__,
                                             "pprm->overlap_start");
  pprm->i_overlap = gal_data_alloc(NULL, block->type, ndim, dsize, NULL, 0, -1,
                                   1, NULL, NULL, NULL);
  pprm->k_overlap = gal_data_alloc(NULL, cprm->kernel->type, ndim, dsize, NULL,
...
gal_convolve_spatial_general(gal_data_t *tiles, gal_data_t *kernel,
  ...
  if (tiles->ndim != kernel->ndim)
    error(EXIT_FAILURE, 0,
          "%s: The number of dimensions between the kernel "
          "and input should be the same",
          __func__);
  if (block->type != GAL_TYPE_FLOAT32 || kernel->type != GAL_TYPE_FLOAT32)
    error(EXIT_FAILURE, 0,
          "%s: only accepts 'float32' type input and "
          "kernel currently",
          __func__);
  ...
  if (tiles->block == NULL && tiles->next && tiles->next->block == NULL)
    error(EXIT_FAILURE, 0,
          "%s: the input is a linked list but not a "
  ...
  if (tocorrect)
    ...
    out->flag =
        (block->flag | (GAL_DATA_FLAG_BLANK_CH | GAL_DATA_FLAG_HASBLANK));
  }
  ...
  if (params.pprm == NULL)
    error(EXIT_FAILURE, 0, "%s: %zu bytes for 'params.pprm'", __func__,
          numthreads * sizeof *params.pprm);
...
gal_data_t *gal_convolve_spatial(gal_data_t *tiles, gal_data_t *kernel,
                                 size_t numthreads, int edgecorrection,
  ...
  return gal_convolve_spatial_general(tiles, kernel, numthreads, edgecorrection,
                                      convoverch, conv_on_blank, NULL);
}
...
void gal_convolve_spatial_correct_ch_edge(gal_data_t *tiles, gal_data_t *kernel,
                                          size_t numthreads, int edgecorrection,
                                          int conv_on_blank,
                                          gal_data_t *tocorrect) {
  ...
  if (gal_dimension_is_different(block, tocorrect))
    error(EXIT_FAILURE, 0,
          "%s: the 'tocorrect' dataset has to have the "
          "same dimensions/size as the block of the 'tiles' input",
          __func__);
  if (block->type != tocorrect->type)
    error(EXIT_FAILURE, 0,
          "%s: the 'tocorrect' dataset has to have the same "
          "type as the block of the 'tiles' input. The given types are '%s' "
          "and '%s' respectively",
          __func__, gal_type_name(tocorrect->type, 1),
          gal_type_name(block->type, 1));
