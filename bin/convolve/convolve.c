...
complextoreal (double *c, size_t size, int action, double **output)
{
  ...
  *output = out
      = gal_pointer_allocate (GAL_TYPE_FLOAT64, size, 0, __func__, "output");
...
correctdeconvolve (struct convolveparams *p, double **spatial)
{
  ...
  if (ps0 % 2 || ps1 % 2)
    error (EXIT_FAILURE, 0,
           "%s: a bug! Please contact us at %s. The padded "
  ...
  if (n == NULL)
    error (EXIT_FAILURE, errno, "%s: allocating %zu bytes for 'n'", __func__,
           ps0 * ps1 * sizeof *n);
                 ...
                 int forward1backwardn1)
{
  ...
  mmapname = gal_threads_dist_in_threads (multiple * p->ps0, nt,
                                          p->input->minmapsize,
                                          p->cp.quietmmap, &indexs, &thrdcols);
  if (nt == 1)
    {
      fp[0].stride = 1;
        ...
        if (indexs[i * thrdcols] != GAL_BLANK_SIZE_T)
          {
            ...
            if (err)
              error (EXIT_FAILURE, 0,
                     "%s: can't create thread %zu for columns", __func__, i);
          }
      pthread_barrier_wait (&b);
      pthread_attr_destroy (&attr);
      pthread_barrier_destroy (&b);
...
convolve_spatial (struct convolveparams *p)
{
  ...
  out = gal_convolve_spatial (
      multidim ? cp->tl.tiles : p->input, p->kernel, cp->numthreads,
      multidim ? !p->noedgecorrection : 1, multidim ? cp->tl.workoverch : 1,
      p->conv_on_blank);
